...
public class SoundEffect implements RSSoundEffect {
	...
	public RawSound toRawSound() {
		...
		return new RawSound(22050, var1, this.start * 22050 / 1000, this.end * 22050 / 1000); // L:
																								// 34
	}
	public final int calculateDelay() {
		...
		for (var2 = 0; var2 < 10; ++var2) { // L: 39
			if (this.instruments[var2] != null && this.instruments[var2].offset / 20 < var1) { // L:
																								// 40
																								// 41
	...
	final byte[] mix() {
		...
		for (var2 = 0; var2 < 10; ++var2) { // L: 58
			if (this.instruments[var2] != null && this.instruments[var2].duration + this.instruments[var2].offset > var1) { // L:
																															// 59
																															// 60
		...
		if (var1 == 0) { // L: 63
			...
			for (int var4 = 0; var4 < 10; ++var4) { // L: 66
				if (this.instruments[var4] != null) { // L: 67
					int var5 = this.instruments[var4].duration * 22050 / 1000; // L:
																				// 68
					int var6 = this.instruments[var4].offset * 22050 / 1000; // L:
																				// 69
					int[] var7 = this.instruments[var4].synthesize(var5, this.instruments[var4].duration); // L:
																											// 70
					for (int var8 = 0; var8 < var5; ++var8) { // L: 71
						int var9 = (var7[var8] >> 8) + var3[var8 + var6]; // L:
																			// 72
