...
public class AudioFilter implements RSAudioFilter {
	private static float[][] field413 = new float[2][8]; // L: 13
	static int[][] coefficients = new int[2][8]; // L: 14
	private static float field409;
	...
	float method1022(int var1, int var2, float var3) {
		float var4 = (float) this.field407[var1][0][var2] + var3 * (float) (this.field407[var1][1][var2] - this.field407[var1][0][var2]); // L:
																																			// 24
		var4 *= 0.0015258789F; // L: 25
		return 1.0F - (float) Math.pow(10.0D, (double) (-var4 / 20.0F)); // L:
																			// 26
	}
	float method1023(int var1, int var2, float var3) {
		float var4 = (float) this.field406[var1][0][var2] + var3 * (float) (this.field406[var1][1][var2] - this.field406[var1][0][var2]); // L:
																																			// 35
	...
	int compute(int var1, float var2) {
		float var3;
		if (var1 == 0) { // L: 41
			var3 = (float) this.field408[0] + (float) (this.field408[1] - this.field408[0]) * var2; // L:
																									// 42
		...
		if (this.pairs[var1] == 0) { // L: 47
			...
			field413[var1][0] = -2.0F * var3 * (float) Math.cos((double) this.method1023(var1, 0, var2)); // L:
																											// 49
			...
			for (var4 = 1; var4 < this.pairs[var1]; ++var4) { // L: 51
				...
				float var5 = -2.0F * var3 * (float) Math.cos((double) this.method1023(var1, var4, var2)); // L:
																											// 53
				float var6 = var3 * var3; // L: 54
				field413[var1][var4 * 2 + 1] = field413[var1][var4 * 2 - 1] * var6; // L:
																					// 55
				field413[var1][var4 * 2] = field413[var1][var4 * 2 - 1] * var5 + field413[var1][var4 * 2 - 2] * var6; // L:
				// 56
		...
		if (var3 != 0) { // L: 70
			...
			for (var5 = 0; var5 < 2; ++var5) { // L: 74
				for (var6 = 0; var6 < this.pairs[var5]; ++var6) { // L: 75
					this.field406[var5][0][var6] = var1.readUnsignedShort(); // L:
																				// 76
					this.field407[var5][0][var6] = var1.readUnsignedShort(); // L:
																				// 77
				}
			}
			for (var5 = 0; var5 < 2; ++var5) { // L: 80
				for (var6 = 0; var6 < this.pairs[var5]; ++var6) { // L: 81
					if ((var7 & 1 << var5 * 4 << var6) != 0) { // L: 82
						this.field406[var5][1][var6] = var1.readUnsignedShort(); // L:
																					// 83
						this.field407[var5][1][var6] = var1.readUnsignedShort(); // L:
																					// 84
					} else {
						this.field406[var5][1][var6] = this.field406[var5][0][var6]; // L:
																						// 87
						this.field407[var5][1][var6] = this.field407[var5][0][var6]; // L:
																						// 88
					}
				}
			}
	...
	private static float normalize(float var0) {
		float var1 = 32.703197F * (float) Math.pow(2.0D, (double) var0); // L:
																			// 30
