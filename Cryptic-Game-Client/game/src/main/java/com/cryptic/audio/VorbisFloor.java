...
public class VorbisFloor implements RSVorbisFloor {
	private static final int[] field264;
	private static final float[] VorbisFloor_decibelstatics;
	private static int[] field271;
	private static int[] field272;
	private static boolean[] field273;
	int[] field262;
	int multiplier;
	...
	VorbisFloor() {
		...
		if (var1 != 1) { // L: 109
			...
			for (var4 = 0; var4 < var3; ++var4) { // L: 124
				this.classDimensions[var4] = VorbisSample.readBits(3) + 1; // L:
																			// 125
				var5 = this.classSubClasses[var4] = VorbisSample.readBits(2); // L:
																				// 126
				if (var5 != 0) { // L: 127
			...
			for (var6 = 0; var6 < var2; ++var6) { // L: 141
				...
				for (int var8 = 0; var8 < this.classDimensions[var7]; ++var8) { // L:
																				// 143
	...
	boolean readSubmapFloor() {
		...
		if (!var1) { // L: 154
			...
			int var4 = StaticSound.iLog(var3 - 1); // L: 158
			field272[0] = VorbisSample.readBits(var4); // L: 159
			field272[1] = VorbisSample.readBits(var4); // L: 160
			int var5 = 2; // L: 161
			for (int var6 = 0; var6 < this.partitionClassList.length; ++var6) { // L:
																				// 162
				...
				for (int var12 = 0; var12 < var8; ++var12) { // L: 169
					int var13 = this.subclassBooks[var7][var11 & var10]; // L:
																			// 170
					var11 >>>= var9; // L: 171
					field272[var5++] = var13 >= 0 ? VorbisSample.VorbisSample_codebooks[var13].method956() : 0; // L:
					// 172
				}
			}
			return true; // L: 175
	...
	void method643(float[] var1, int var2) {
		...
		for (var6 = 2; var6 < var3; ++var6) { // L: 184
			...
			var9 = this.method638(field271[var7], field272[var7], field271[var8], field272[var8], field271[var6]); // L:
			// 187
			...
			if (var10 != 0) { // L: 192
				...
				if (var10 >= var13) { // L: 198
					field272[var6] = var11 > var9 ? var9 + (var10 - var9) : var11 + (var9 - var10) - 1; // L:
																										// 199
				} else {
					field272[var6] = (var10 & 1) != 0 ? var9 - (var10 + 1) / 2 : var10 / 2 + var9; // L:
																									// 202
				}
			} else {
	...
	private static int method656(int[] var0, int var1) {
	...
	private static int method637(int[] var0, int var1) {
