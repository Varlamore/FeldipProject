...
public abstract class AbstractFont extends Rasterizer2D {
    ...
    public int stringWidthNoFormatting(String var1) {
        ...
        // Remove '<col=...>...</col>' patterns, keeping the text inside
        var1 = var1.replaceAll("<[^>]*>", "");
        return stringWidth(var1);
    }
    ...
    public int method1905(CharSequence arg0) {
        return method120(arg0, 10, true);
    }

    public int method120(CharSequence arg0, int arg1, boolean arg2) {
        if (arg1 >= 2 && arg1 <= 36) {
            boolean var4 = false;
            boolean var5 = false;
            int var6 = 0;
            ...
            for(int index = 0; index < textLength; ++index) {
                ...
                if (var15 == '<') {
                    ...
                    if (var15 == '>' && tagStart != -1) {
                        ...
                        lineBuilder.append('>');
                        if (tag.equals("br")) {
                            lines[lineIndex] = lineBuilder.substring(lineStart, lineBuilder.length());
                            ++lineIndex;
                    ...
                    if (tagStart == -1) {
                        ...
                        if (widths != null && charCount > widths[lineIndex < widths.length ? lineIndex : widths.length - 1] && spacePos >= 0) {
                            lines[lineIndex] = lineBuilder.substring(lineStart, spacePos - addSpace);
                            ++lineIndex;
                            lineStart = spacePos;
                            spacePos = -1;
                    ...
                    if (var15 == '>' && tagStart != -1) {
                        ...
                        if (tag.equals("br")) {
                            ...
                            try {
                                ...
                                //Added check for null to prevent clan chat NPE
                                if (chat_emblems != null && chat_emblems[id] != null) {
                                    charCount += chat_emblems[id].width;
                                }
                                //If it's not a chat emblem sprite, let's try it as a regular sprite.
                                else if (SpriteCache.get(id) != null) {
                                    charCount += Objects.requireNonNull(SpriteCache.get(id)).width;
                                }
                                prevChar = 0;
                            } catch (Exception var20) {
                            }
    ...
    public void draw(String string, int drawX, int drawY, int color, int shadow, int alpha) {
        if (string != null) {
            this.reset(color, shadow);
    ...
    public void draw(String string, int drawX, int drawY, int color, boolean shadow) {
        draw(string,drawX,drawY,color,shadow ? 1 : 0);
    }
    ...
    public void drawCenteredString(String string, int drawX, int drawY, int color, int shadow, int alpha) {
        if (string != null) {
            this.reset(color, shadow);
            AbstractFont.alpha = alpha;
    ...
    public void drawCenteredString(String string, int drawX, int drawY, int color, boolean shadow) {
        drawCenteredString(string,drawX,drawY,color,shadow ? 1 : 0);
    }
                            ...
                            if (!var8.equals("gt")) {
                                if (var8.startsWith("img=")) {
                                    try {
                                        ...
                                        int id = Integer.parseInt(var8.substring(4));
                                        ...
                                        int height_offset = icon.height - 1;//11;//changed to max_height
                                        icon.drawAdvancedSprite(drawX, (drawY + this.ascent - height_offset));
                                        drawX += icon.width + icon.max_width / drawX;
                                        var5 = -1;
                                    } catch (Exception exception) {
                                        exception.printStackTrace();
                                        Client.addReportToServer(exception.getMessage());
                            ...
                            if (!var11.equals("gt")) {
                                if (var11.startsWith("img=")) {
                                    try {
                                        ...
                                        //System.out.println("effect is: " + effect);
                                        int id = Integer.parseInt(var11.substring(4));
                                        //System.out.println("id is: " + id);
                                        SimpleImage icon = null;
                                        if(id != -1) {
                                            icon = SpriteCache.get(id);
                                        ...
                                        icon.drawAdvancedSprite(var12 + var2, var13 + (var3 + this.ascent - height_offset));
                                        var2 += icon.width + icon.max_width / var2;
                                        var7 = -1;
                                    } catch (Exception exception) {
                                        exception.printStackTrace();
                                        Client.addReportToServer(exception.getMessage());
