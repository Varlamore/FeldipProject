...
public class AnimationKeyFrame {
   ...
   private boolean interpolateTiming;
   private boolean dirty = true;
   private boolean isConstant;
   private InterpolationType interpolationType1;
   private InterpolationType interpolationType;
   private AnimKey[] keyframes;
   private float start;
   private float end;
   private float minimumValue;
   private float maximumValue;
   private float[] interpolatedTimeBuffer = new float[4];
   private float[] interpolatedValueBuffer = new float[4];
   private float[] values;
   private int cached_keyframe_id = 0;
   private int startTick;
   private int endTick;

   public int deserialise(Buffer packet,int version) {
      int mutationsCount = packet.readUShort();
      ...
      this.keyframes = new AnimKey[mutationsCount];
      AnimKey lastKeyframe = null;


      for(int index = 0; index < mutationsCount; ++index) {
      ...
      this.startTick = this.keyframes[0].tick;
      this.endTick = this.keyframes[this.getKeyframesCount() - 1].tick;
   ...
   public void deserialise() {
      ...
      for (int var1 = this.getStartTick(); var1 <= this.getEndTick(); ++var1) {
         this.values[var1 - this.getStartTick()] = calculateInterpolatedValue(this, (float) var1);
      }
   ...
   private int getKeyframeByTick(float tick) {
   ...
   private AnimKey getKeyframe(float value) {
   ...
   private static float calculateInterpolatedValue(AnimationKeyFrame operation, float tick) {
   ...
   private static float calculateAtPoint(AnimationKeyFrame operation, float current_tick, boolean forward) {
      if (operation == null || operation.getKeyframesCount() == 0) {
         return 0.0F;
      }
    ...
    */
   private static void calcCubicsplineShapeCoefficients(AnimationKeyFrame operation, float[] time, float[] values) {
      if (operation != null) {
   ...
   private static float interpolateShape(AnimationKeyFrame operation, float tick) {
      if (operation == null) {
         return 0.0F;
      } else {
   ...
   private static float interpolateTiming(AnimationKeyFrame operation, float tick) {
      if (operation == null) {
         return 0.0F;
      }
      float elapsed;
   ...
   private static int linearInterpolate(float[] keyframe_values, int keyframe_count, float start_tick, boolean clampstart, float end_tick, boolean clampend, float[] interpolated_values) {
