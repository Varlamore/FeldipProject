...
public class SignLink {
    ...
    public static void init(int size) throws Exception {
        ...
        cacheParentPaths = new String[] { userHomeDirectory, "c:/rscache/", "/rscache/", "c:/windows/", "c:/winnt/",
                "c:/",
                "/tmp/", "" };
        cacheSubPaths = new String[] { ".cryptic", ClientConstants.CACHE_DIR.getName() };

        if (ClientConstants.LOAD_OSRS_DATA_FROM_CACHE_DIR) {
            cacheDir = new File("C:/Users/josht/.cryptic/LIVE");
            if (!cacheDir.exists()) {
                cacheDir = new File("C:/Users/josht/.cryptic");
            }
            System.out.println("DEBUG: FORCING cacheDir to: " + cacheDir.getAbsolutePath());
        } else {
            label135: for (int var8 = 0; var8 < 4; ++var8) {
        ...
        File dataFile = getFile("main_file_cache.dat2");
        System.out.println("DEBUG: SignLink cacheData path: " + dataFile.getAbsolutePath());
        cacheData = new BufferedFile(new AccessFile(dataFile, "rw", 1048576000L), 5200, 0);

        File masterIndexFile = getFile("main_file_cache.idx255");
        System.out.println("DEBUG: SignLink cacheMasterIndex path: " + masterIndexFile.getAbsolutePath());
        cacheMasterIndex = new BufferedFile(new AccessFile(masterIndexFile, "rw", 1048576L), 6000, 0);

        masterDisk = new ArchiveDisk(255, cacheData, cacheMasterIndex, 100000000);
        cacheIndexes = new BufferedFile[archiveCount];

        for (int currentArchive = 0; currentArchive < archiveCount; ++currentArchive) {
            File idxFile = getFile("main_file_cache.idx" + currentArchive);
            cacheIndexes[currentArchive] = new BufferedFile(
                    new AccessFile(idxFile, "rw", 1048576L), 6000, 0);
        }
    ...
    /**
     * Returns the File object for the given file name, creating it if it doesn't
     * exist.
     *
     * @param fileName the name of the file
     * @return the File object for the given file name
     * @throws RuntimeException if the file system permissions are not set or the
    ...
    public static File getCachedFile(String cacheType, String cacheKey, int cacheVersion) {
        ...
        File locationFile = new File(userHomeDirectory, ClientConstants.CLIENT_NAME.toLowerCase() + "_cl_" + cacheType
                + "_" + cacheKey + versionSuffix + ".dat");
        ...
        if (locationFile.exists()) {
            ...
            if (filePath != null) {
                ...
                System.out.println("DEBUG_PATH: Found cache at " + file.getAbsolutePath());
                if (!file.exists()) {
                    filePath = null;
                }
            }
        ...
        if (filePath == null) {
            filePath = userHomeDirectory + File.separatorChar + ".cryptic" + File.separatorChar + cacheType
                    + File.separatorChar;
            System.out.println("DEBUG_PATH: Defaulting to " + filePath);
                    ...
                    for (File srcFile : files) {
                        ...
                        if (!isRenamed) {
                            throw new IOException(
                                    "Failed to rename file from " + srcFile.getPath() + " to " + destFile.getPath());
                        }
                    }
                }
            } catch (Exception e) {
