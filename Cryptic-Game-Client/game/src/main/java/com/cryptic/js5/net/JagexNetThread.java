...
import com.cryptic.collection.node.DualNodeDeque;
...
public class JagexNetThread implements RSNetCache {
                ...
                if (field3586 > 30000) {
                    ...
                    for (int loopCount = 0; loopCount < 100; ++loopCount) {
                        ...
                        if (controlPacketType > 0) {
                            bytesRead = controlPacketType - field3605.pos;
                            if (bytesRead > available) {
                                bytesRead = available;
                            }

                            socket.read(field3605.payload, field3605.pos, bytesRead);
                            if (field3584 != 0) {
                                for (maxBytesToRead = 0; maxBytesToRead < bytesRead; ++maxBytesToRead) {
                                    payload = field3605.payload;
                                    payloadIndex = maxBytesToRead + field3605.pos;
                            ...
                            if (field3605.pos < controlPacketType) {
                                break;
                            }
                            ...
                            if (field3596 == null) {
                                ...
                                data = field3605.readUnsignedShort();
                            ...
                            if (maxBytesToRead > bytesRead - responsiveArchiveBuffer.pos) {
                                maxBytesToRead = bytesRead - responsiveArchiveBuffer.pos;
                            }
                            ...
                            socket.read(responsiveArchiveBuffer.payload, responsiveArchiveBuffer.pos, maxBytesToRead);

                            if (field3584 != 0) {
                                for (data = 0; data < maxBytesToRead; ++data) {
                                    payload = responsiveArchiveBuffer.payload;
                                    payloadIndex = data + responsiveArchiveBuffer.pos;
                                    payload[payloadIndex] ^= field3584;
                                }
                            }
                            ...
                            if (responsiveArchiveBuffer.pos == bytesRead) {
    ...
    public void loadArchiveIndex(Js5Archive archive, int archiveIndex) {
        field3603.pos = archiveIndex * 8 + 5;

        if (field3603.pos >= field3603.payload.length) {
