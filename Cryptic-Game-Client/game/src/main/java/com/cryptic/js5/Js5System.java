...
import static com.cryptic.ClientConstants.*;
...
public class Js5System {
    ...
    public static void doCycleJs5() {
        if (Client.instance.gameState != 1000) {
            ...
            if (!idle) {
                // System.out.println("js5 loaded");
                connect();
            }
        }
    }

    public static Js5Archive createJs5(Js5ArchiveIndex archive, boolean useWhirlpool, boolean useGzip,
            boolean verifyChecksums, boolean prioritizeRequests) {
        ArchiveDisk cache = null;
        if (SignLink.cacheData != null) {
            cache = new ArchiveDisk(archive.getId(), SignLink.cacheData, SignLink.cacheIndexes[archive.getId()],
                    100000000);
        }

        Js5Archive js5Archive = new Js5Archive(cache, SignLink.masterDisk, archive, useWhirlpool, useGzip,
                verifyChecksums,
                prioritizeRequests);
        if (LOAD_OSRS_DATA_FROM_CACHE_DIR) {
            js5Archive.loadLocal();
        }
        return js5Archive;
    }

    public static void init(Js5Archive js5Archive, String name) {
        if (!js5Archive.hasFiles()) {
            js5Archive.loadLocal(); // Try loading again just in case?
            if (!js5Archive.hasFiles()) {
                return;
            }
        }
            ...
            if (--js5Cycles + 1 <= 0) {
                try {
                    if (js5ConnectState == 0) {
                        js5SocketTask = GameEngine.taskHandler.newSocketTask(SERVICES_IP, SERVICES_PORT);
                        ++js5ConnectState;
