...
public class CacheDownloader implements Runnable {
    ...
    private static final String CACHE_DOWNLOAD_LINK = "https://www.dropbox.com/s/0t79urkalk0qbzj/.Zaryte202.zip?dl=1";
    ...
    public CacheDownloader downloadCache() {
        try {
            ...
            cacheVersionRemote = Integer.parseInt(ClientConstants.CLIENT_VERSION);
    ...
    private void update() {
        ...
        Unzip.unZipIt(fileLocation.toString(), SignLink.findCacheDir(), true);
        writeVersion(cacheVersionRemote);
        deleteZip();
    }

    private void downloadFile(String localFileName) {
        try (OutputStream out = new BufferedOutputStream(Files.newOutputStream(Paths.get(SignLink.getCacheDirectory() + "/" + localFileName)))) {
            ...
            try (InputStream in = conn.getInputStream()) {
                ...
                while ((numRead = in.read(data)) != -1) {
                    ...
                    client.draw_loadup(percentage, ClientConstants.CLIENT_NAME + " - Downloading Cache " + percentage + "%");
                }
            }
            out.flush();
        } catch (IOException ex) {
