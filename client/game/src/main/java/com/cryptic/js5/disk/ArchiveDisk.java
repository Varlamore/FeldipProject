        ...
        synchronized (dataFile) {
            try {
                if (indexFile.length() < (long) groupId * INDEX_BLOCK_SIZE + INDEX_BLOCK_SIZE) {
                    System.out.println("DEBUG_DISK: archive=" + archiveid + " groupId=" + groupId
                            + " FAILED (index too small: " + indexFile.length() + ")");
                    return null;
                }
                ...
                if (size < 0 || size > maxSize) {
                    System.out.println("DEBUG_DISK: archive=" + archiveid + " groupId=" + groupId
                            + " FAILED (invalid size: " + size + ")");
                    return null;
                }
                if (block <= 0 || (long) block > dataFile.length() / DATA_BLOCK_SIZE) {
                    System.out.println("DEBUG_DISK: archive=" + archiveid + " groupId=" + groupId
                            + " FAILED (invalid block: " + block + " dataFileLen=" + dataFile.length() + ")");
                    return null;
                }
