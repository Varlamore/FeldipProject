...
import com.cryptic.collection.table.IntHashTable;
import com.cryptic.io.Buffer;
import com.cryptic.js5.util.Js5ConfigType;
import com.cryptic.net.AbstractByteArrayCopier;
import com.cryptic.net.DirectByteArrayCopier;
import com.cryptic.util.StringUtils;
import com.cryptic.util.ziptools.BZip2Decompressor;
import com.cryptic.util.ziptools.CRC32;
import com.cryptic.util.ziptools.GzDecompressor;
...
public abstract class AbstractArchive {
        ...
        if (protocol >= 5 && protocol <= 7) {
            ...
            if (protocol >= 7) {
                for (int index = 0; index < this.groupCount; ++index) {
                    ...
                    if (this.groupIds[index] > lastGroupId) {
                        lastGroupId = this.groupIds[index];
                    }
                }
            } else {
                for (int index = 0; index < this.groupCount; ++index) {
            ...
            if (var6 != 0) {
                ...
                for (int index = 0; index < this.groupCount; ++index) {
                    this.groupNameHashes[this.groupIds[index]] = buffer.readInt();
                }
            ...
            for (int index = 0; index < this.groupCount; ++index) {
                this.fileCounts[this.groupIds[index]] = buffer.get_unsignedshort();
            }
    ...
    public byte[] takeFile(Js5ConfigType var1, int var2) {
        return this.takeFileEncrypted(var1.getId(), var2, (int[]) null);
    }
    ...
    public byte[] takeFileFlat(int file) {
        if (this.files.length == 1) {
            return this.takeFile(0, file);
        } else if (this.files[file].length == 1) {
            return this.takeFile(file, 0);
        } else {
    ...
    public byte[] getFileFlat(int file) {
        if (this.files.length == 1) {
            return this.getFile(0, file);
        } else if (this.files[file].length == 1) {
            return this.getFile(file, 0);
        } else {
            throw new RuntimeException("Unable to take File: " + file);
        }
    }
                ...
                if (fileCount > 1) {
                    ...
                    for (int fileIndex = 0; fileIndex < fileCount; ++fileIndex) {
                        if (!this.shallowFiles) {
                            ...
                            fileArray[fileId[fileIndex]] = var23[fileIndex];
                        }
                    }
    ...
    public static final byte[] decompressBytes(byte[] var0) {
        ...
        if (var3 >= 0 && (AbstractArchive.maxSize == 0 || var3 <= AbstractArchive.maxSize)) {
            if (var2 == 0) {
                ...
                if (var6 < 0 || AbstractArchive.maxSize != 0 && var6 > AbstractArchive.maxSize) {
                    throw new RuntimeException();
                } else {
