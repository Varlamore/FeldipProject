...
public class Js5Archive extends AbstractArchive implements RSArchive {
    ...
    public Js5Archive(ArchiveDisk archiveDisk, ArchiveDisk masterDisk, Js5ArchiveIndex js5ArchiveIndex,
            boolean useWhirlpool, boolean useGzip, boolean verifyChecksums, boolean prioritizeRequests) {
            ...
            if (var3 != null && var3.length > 2) {
                ...
                if (var5 == super.groupCrcs[var2] && var11 == super.groupVersions[var2]) {
                    ...
                    if (verifyChecksums || var4) {
                        Client.jagexNetThread.requestNetFile(this, this.index, var2, super.groupCrcs[var2], (byte) 2,
                                var4);
                    }
    ...
    public boolean hasFiles() {
        return super.files != null;
    }

    public void loadLocal() {
        if (this.masterDisk != null) {
            byte[] data = this.masterDisk.read(this.index);
            if (data != null) {
                this.decodeIndex(data);
                this.loadAllLocal();
            } else {
                System.out.println("DEBUG: loadLocal FAILED (data null) for index " + this.index);
            }
        } else {
            System.out.println("DEBUG: loadLocal FAILED (masterDisk null) for index " + this.index);
        }
    }
        ...
        if (this.loading != 1 && (!this.prioritizeRequests || this.loading != 2)) {
            if (super.groups != null) {
                ...
                if (Client.jagexNetThread.field3596 != null && Client.jagexNetThread.field3596.key == key) {
                    percentage = Client.jagexNetThread.responsiveArchiveBuffer.pos * 99
                            / (Client.jagexNetThread.responsiveArchiveBuffer.payload.length
                                    - Client.jagexNetThread.field3596.padding)
                            + 1;
                } else {
                    percentage = 0;
                }
    ...
    void loadGroup(int group) {
        if (this.archiveDisk != null && this.validGroups != null && this.validGroups[group]) {
            ...
            synchronized (ArchiveDiskActionHandler.requestQueue) {
                for (ArchiveDiskAction action = (ArchiveDiskAction) ArchiveDiskActionHandler.requestQueue
                        .last(); action != null; action = (ArchiveDiskAction) ArchiveDiskActionHandler.requestQueue
                                .previous()) {
    ...
    int groupLoadPercent(int groupIndex) {
        ...
        if (Client.jagexNetThread.field3596 != null && Client.jagexNetThread.field3596.key == groupKey) {
            int bufferLength = Client.jagexNetThread.responsiveArchiveBuffer.payload.length
                    - Client.jagexNetThread.field3596.padding;
