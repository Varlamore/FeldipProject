...
public class VorbisCodebook implements RSVorbisCodebook {
	...
	VorbisCodebook() {
		...
		if (var1) { // L: 46
			var2 = 0; // L: 47
			for (var3 = VorbisSample.readBits(5) + 1; var2 < this.entries; ++var3) { // L:
																						// 48
																						// 49
																						// 52
				int var4 = VorbisSample.readBits(StaticSound.iLog(this.entries - var2)); // L:
																						// 50
			...
			for (var3 = 0; var3 < this.entries; ++var3) { // L: 57
				if (var14 && VorbisSample.readBit() == 0) { // L: 58
					...
					this.lengthMap[var3] = VorbisSample.readBits(5) + 1; // L:
																			// 59
				}
			}
		}
		...
		if (var2 > 0) { // L: 64
			float var15 = VorbisSample.float32Unpack(VorbisSample.readBits(32)); // L:
																					// 65
			float var16 = VorbisSample.float32Unpack(VorbisSample.readBits(32)); // L:
																					// 66
			...
			if (var2 == 1) { // L: 75
				for (var8 = 0; var8 < this.entries; ++var8) { // L: 76
					...
					for (var11 = 0; var11 < this.dimensions; ++var11) { // L: 79
						...
						float var13 = (float) this.field357[var12] * var16 + var15 + var9; // L:
																							// 81
				...
				for (var8 = 0; var8 < this.entries; ++var8) { // L: 89
					...
					for (var11 = 0; var11 < this.dimensions; ++var11) { // L: 92
						float var17 = (float) this.field357[var10] * var16 + var15 + var9; // L:
																							// 93
			...
			if (var4 != 0) { // L: 145
				...
				for (var7 = 0; var7 < var4; ++var7) { // L: 148
					...
					if (var6 >= this.keys.length) { // L: 157
						...
						for (var10 = 0; var10 < this.keys.length; ++var10) { // L:
																				// 159
	...
	int method956() {
		int var1;
		for (var1 = 0; this.keys[var1] >= 0; var1 = VorbisSample.readBit() != 0 ? this.keys[var1] : var1 + 1) { // L:
			// 170
			// 171
		}
	...
	private static int mapType1QuantValues(int var0, int var1) {
		int var2 = (int) Math.pow((double) var0, 1.0D / (double) var1) + 1; // L:
																			// 16
		while (true) {
			int var4 = var2; // L: 19
			int var5 = var1; // L: 20
