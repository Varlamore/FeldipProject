...
public class Instrument implements RSInstrument {
	private static int[] Instrument_samples;
	private static int[] Instrument_noise;
	private static int[] Instrument_sine;
	private static int[] Instrument_phases;
	private static int[] Instrument_delays;
	private static int[] Instrument_volumeSteps;
	private static int[] Instrument_pitchSteps;
	private static int[] Instrument_pitchBaseSteps;
	...
	static {
		...
		for (var1 = 0; var1 < 32768; ++var1) { // L: 37
			Instrument_sine[var1] = (int) (Math.sin((double) var1 / 5215.1903D) * 16384.0D); // L:
																								// 38
		}
	...
	final int[] synthesize(int var1, int var2) {
		StaticSound.clearIntArray(Instrument_samples, 0, var1); // L: 53
		if (var2 < 10) { // L: 54
			...
			if (this.pitchModifier != null) { // L: 61
				...
				var5 = (int) ((double) (this.pitchModifier.end - this.pitchModifier.start) * 32.768D / var3); // L:
																												// 64
				var6 = (int) ((double) this.pitchModifier.start * 32.768D / var3); // L:
																					// 65
			}
			int var8 = 0; // L: 67
			int var9 = 0; // L: 68
			...
			if (this.volumeMultiplier != null) { // L: 70
				...
				var8 = (int) ((double) (this.volumeMultiplier.end - this.volumeMultiplier.start) * 32.768D / var3); // L:
				// 73
				var9 = (int) ((double) this.volumeMultiplier.start * 32.768D / var3); // L:
																						// 74
			}
			int var11;
			for (var11 = 0; var11 < 5; ++var11) { // L: 76
				if (this.oscillatorVolume[var11] != 0) { // L: 77
					...
					Instrument_delays[var11] = (int) ((double) this.oscillatorDelays[var11] * var3); // L:
																										// 79
					Instrument_volumeSteps[var11] = (this.oscillatorVolume[var11] << 14) / 100; // L:
																								// 80
					Instrument_pitchSteps[var11] = (int) ((double) (this.pitch.end - this.pitch.start) * 32.768D * Math.pow(1.0057929410678534D, (double) this.oscillatorPitch[var11]) / var3); // L:
																																																// 81
					Instrument_pitchBaseSteps[var11] = (int) ((double) this.pitch.start * 32.768D / var3); // L:
																											// 82
				}
			}
			int var12;
			...
			for (var11 = 0; var11 < var1; ++var11) { // L: 84
				...
				if (this.pitchModifier != null) { // L: 87
					...
					var12 += this.evaluateWave(var7, var15, this.pitchModifier.form) >> 1; // L:
																							// 90
					var7 = var7 + var6 + (var14 * var5 >> 16); // L: 91
				}
				if (this.volumeMultiplier != null) { // L: 93
					var14 = this.volumeMultiplier.doStep(var1); // L: 94
					var15 = this.volumeMultiplierAmplitude.doStep(var1); // L:
																			// 95
					var13 = var13 * ((this.evaluateWave(var10, var15, this.volumeMultiplier.form) >> 1) + 32768) >> 15; // L:
					// 96
			...
			if (this.release != null) { // L: 108
				...
				for (var14 = 0; var14 < var1; ++var14) { // L: 114
					...
					if (var20) { // L: 117
						...
						var12 = (var16 * (this.release.end - this.release.start) >> 8) + this.release.start; // L:
																												// 118
					}
					var11 += 256; // L: 119
					if (var11 >= var12) {
			...
			if (this.filter.pairs[0] > 0 || this.filter.pairs[1] > 0) { // L:
																		// 130
				this.filterEnvelope.reset(); // L: 131
				var11 = this.filterEnvelope.doStep(var1 + 1); // L: 132
				var12 = this.filter.compute(0, (float) var11 / 65536.0F); // L:
																			// 133
				var13 = this.filter.compute(1, (float) var11 / 65536.0F); // L:
																			// 134
				if (var1 >= var12 + var13) { // L: 135
					...
					int var17;
					while (var14 < var15) {
						var16 = (int) ((long) Instrument_samples[var14 + var12] * (long) AudioFilter.forwardMultiplier >> 16); // L:
																																// 140
					...
					while (true) {
						...
						int var18;
						while (var14 < var15) {
							var17 = (int) ((long) Instrument_samples[var14 + var12] * (long) AudioFilter.forwardMultiplier >> 16); // L:
																																	// 151
							...
							var11 = this.filterEnvelope.doStep(var1 + 1); // L:
																			// 155
							++var14; // L: 150
						}
						if (var14 >= var1 - var12) { // L: 157
							while (var14 < var1) {
								var17 = 0; // L: 163
								for (var18 = var14 + var12 - var1; var18 < var12; ++var18) { // L:
																								// 164
									var17 += (int) ((long) Instrument_samples[var14 + var12 - 1 - var18] * (long) AudioFilter.coefficients[0][var18] >> 16);
								}
								for (var18 = 0; var18 < var13; ++var18) { // L:
																			// 165
									var17 -= (int) ((long) Instrument_samples[var14 - 1 - var18] * (long) AudioFilter.coefficients[1][var18] >> 16);
								}
	...
	final int evaluateWave(int var1, int var2, int var3) {
		if (var3 == 1) { // L: 179
			...
			return var3 == 4 ? var2 * Instrument_noise[var1 / 2607 & 32767] : 0; // L:
																					// 185
																					// 186
		}
	}
