...
public class BankWidget extends Widget {
    ...
    private static void bank(AdvancedFont[] font) {
        ...
        for (int tab = 0; tab < 40; tab += 4) {
            addButton(interfaceId + 31 + tab, SpriteCache.get(109), SpriteCache.get(109),
        ...
        SimpleImage disabled = SpriteCache.get(766);
        SimpleImage enabled = SpriteCache.get(767);
        ...
        addContainer(5382, 109, 8, 102, 17, 4, "Withdraw-1", "Withdraw-5", "Withdraw-10", "Withdraw-All", "Withdraw-X", "Withdraw-All but one");
        ...
        for (int rsint : interfaces) {
            ...
            cache[rsint].width = Objects.requireNonNull(enabled).width;
            cache[rsint].height = enabled.height;
        }

        cache[8130].enabledSprite = SpriteCache.get(893);
        cache[8130].disabledSprite = SpriteCache.get(894);
        cache[8130].width = 48;
        cache[8130].tooltip = "Swap";

        cache[8131].enabledSprite = SpriteCache.get(893);
        cache[8131].disabledSprite = SpriteCache.get(894);
        cache[8131].x = -9;
        cache[8131].width = 48;
        cache[8131].tooltip = "Insert";

        cache[5387].enabledSprite = SpriteCache.get(893);
        cache[5387].disabledSprite = SpriteCache.get(894);
        cache[5387].x = -21;
        cache[5387].width = 48;
        cache[5387].tooltip = "Item";

        cache[5386].enabledSprite = SpriteCache.get(893);
        cache[5386].disabledSprite = SpriteCache.get(894);
        cache[5386].x = -30;
        cache[5386].width = 48;
