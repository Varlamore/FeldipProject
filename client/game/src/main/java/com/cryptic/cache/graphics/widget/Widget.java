...
import com.cryptic.cache.graphics.widget.impl.accounttabs.AccountTab;
import com.cryptic.cache.graphics.widget.impl.accounttabs.CommunityTab;
import com.cryptic.cache.graphics.widget.impl.accounttabs.LinksTab;
import com.cryptic.cache.graphics.widget.impl.pos.*;
import com.cryptic.cache.graphics.widget.impl.questtabs.ModernAchievementsTab;
import com.cryptic.cache.graphics.widget.impl.questtabs.ModernInformationTab;
import com.cryptic.cache.graphics.widget.impl.questtabs.ModernOSRSQuestTab;
import com.cryptic.cache.graphics.widget.impl.tob.NightmareHealth;
import com.cryptic.cache.graphics.widget.impl.tob.TheatreOfBloodPartyWidget;
...
import com.cryptic.cache.graphics.widget.impl.accountcreation.AccountCreationWidget;
import com.cryptic.cache.graphics.widget.impl.lootkey.LootKeyWidget;
import com.cryptic.cache.graphics.widget.impl.mbox.MysteryBoxWidget;
...
import static com.cryptic.util.Utils.d2Tod1;
 ...
 */
@Slf4j
public class Widget implements RSWidget {
    ...
    public static final int TYPE_WRAPPING_TEXT = 44;
    ...
    public static final int DRAW_BOX = 30;
    public static final int PROGRESS_BAR = 31;
    public static final int HOVER_BUTTONS_NEW = 42;
    public static final int TYPE_DRAW_PIXELS = 43;
    public static boolean PRINT_INFO;
    ...
    @Override
    public void setAction(int idx, String action) {
    ...
    @Override
    public void setOnReleaseListener(Object[] o) {

    }

    @Override
    public void setVarTransmitTrigger(int[] i) {

    }

    @Override
    public void setOnHoldListener(Object[] o) {
    ...
    @Override
    public void setOnDragCompleteListener(Object[] o) {

    }

    @Override
    public void setOnDragListener(Object[] o) {
    ...
    @Override
    public void setDragParent(net.runelite.api.widgets.Widget dragParent) {
    ...
    @Override
    public void setOnVarTransmitListener(Object[] o) {
    ...
    public static void addHoveredButton_sprite_loader(int i, int spriteId, int w, int h, int IMAGEID) {
    ...
    public static void load(Archive interfaces, AdvancedFont font[], Archive graphics) {
        ...
        while (buffer.pos < buffer.payload.length) {
            ...
            if (widget.type == TYPE_MODEL) {
                ...
                widget.modelAngleX = buffer.readUnsignedShort();
                widget.modelAngleY = buffer.readUnsignedShort();
            }
        ...
        new HealthHud().build();
        new ItemStatementContainer().build();
        ...
        AccountCreationWidget.unpack(font);
        ...
        NewTeleportInterface.unpack(font);
        ...
        //Interfaces by Clive/Dinh
        new TheatreOfBloodPartyWidget().build();
        new ModernShopInterface().build();
        new ModernPresetManager().build();
        new ModernOSRSQuestTab().build();
        new ModernInformationTab().build();
        new ModernAchievementsTab().build();
        new DailyTasksInterface().build();
        new AccountTab().build();
        new LinksTab().build();
        new CommunityTab().build();

        new Overview().build();
        new BuyAnItem().build();
        new BuyOverlay().build();
        new TradeHistory().build();
        new RecentListings().build();
        new SellAnItem().build();

        new NightmareHealth().build();
    ...
    //reference Cache
    private static com.cryptic.collection.table.EvictingDualNodeHashTable spriteCache;
    ...
    /**
     * currently clicked rather than enabled, judging by code
     */
    ...
    /**
     * doubt this was vanilla 317 code the fuck is this
     */
    public boolean serverCheck = false;
    public boolean regularHoverBox;
    /**
     * doubt this was vanilla 317 code
     */
    ...
    public static void textClicked(int interfaceId, int spriteId, int xOffset, int yOffset) {
        cache[interfaceId].textSpriteClicked = SpriteCache.get(spriteId);
    ...
    public static void drawProgressBar(int id, int width, int height, int currentPercent, int backingColour, int barColour) {
        Widget rsi = addInterface(id);
        rsi.type = 287;
        rsi.width = width;
        rsi.height = height;
        rsi.currentPercent = currentPercent;
        rsi.backingColour = backingColour;
        rsi.barColour = barColour;
    }

    public static void interpolateProgressBar(int id, int width, int height, int currentPercent, int backingColour, int barColour, byte opacity) {
        Widget rsi = addInterface(id);
        rsi.type = 287;
        rsi.width = width;
        rsi.height = height;
    ...
    public int backingColour = 0;
    public int barColour = 0;

    public static Widget addProgressBar(int id, int width, int height, int current, int max) {
        Widget rsi = addInterface(id);
        rsi.id = id;
        rsi.type = PROGRESS_BAR;
        rsi.width = width;
        rsi.height = height;
        rsi.defaultText = current + "/" + max;
        return rsi;
    }

    public Widget addBox(int id, int width, int height, int border, int borderColor, int color, int transparency) {
    ...
    public static void addSpellSprite(int id, int spriteId) {
        ...
        if (spriteId != -1) {
            widget.enabledSprite = SpriteCache.get(spriteId);
            widget.disabledSprite = SpriteCache.get(spriteId);
        }
    ...
    public static void closeButton(int id, int enabledSprite, int disabledSprite, boolean serverActionRequired) {
        ...
        widget.disabledSprite = SpriteCache.get(enabledSprite);
        widget.enabledSprite = SpriteCache.get(disabledSprite);
        widget.width = SpriteCache.getWidth(enabledSprite);
        widget.height = SpriteCache.getHeight(enabledSprite);
    ...
    public static void hoverButton(int id, String tooltip, int enabledSprite, int disabledSprite) {
        ...
        widget.optionType = 1;
        widget.type = TYPE_HOVER;
        SpriteCache.get(enabledSprite);
        SpriteCache.get(disabledSprite);
        widget.disabledSprite = SpriteCache.get(enabledSprite);
        widget.enabledSprite = SpriteCache.get(disabledSprite);
        widget.width = SpriteCache.getWidth(enabledSprite);
        widget.height = SpriteCache.getHeight(enabledSprite);
     ...
     * @param disabledSprite - The ID of the sprite when the mouse is not over the button
     * @param enabledSprite - The ID of the sprite when the mouse is over the button
     ...
     */
    public static void hoverButton(int id, int disabledSprite, int enabledSprite, String hoverTooltip, int font, int color, String buttonText, boolean shadowText) {
        ...
        tab.enabledSprite = SpriteCache.get(enabledSprite);
        tab.width = SpriteCache.getWidth(enabledSprite);
        tab.height = SpriteCache.getHeight(enabledSprite);
        tab.msgX = tab.width / 2;
        tab.msgY = (tab.height / 2) + 5;
        tab.defaultText = buttonText;
        tab.text_type = fonts[font];
        tab.textColour = color;
        tab.centerText = true;
        tab.toggled = false;
        tab.textShadow = shadowText;
    }
     ...
     */
    public static void hoverButton(int id, int disabledSprite, int enabledSprite, String hoverTooltip, int font, int color, String buttonText, int msgX, int msgY, boolean shadowText) {
        Widget tab = addInterface(id);
        tab.tooltip = hoverTooltip;
        tab.optionType = 1;
        tab.type = 42;
        tab.disabledSprite = SpriteCache.get(disabledSprite);
        tab.enabledSprite = SpriteCache.get(enabledSprite);
        tab.width = SpriteCache.getWidth(disabledSprite);
        tab.height = SpriteCache.getHeight(disabledSprite);
    ...
    public static void addButton(int id, int parent, int width, int height, int config, int configFrame, int sprite1, int sprite2, int hoverOver, String tooltip) {
        ...
        widget.enabledSprite = SpriteCache.get(sprite1);
        widget.disabledSprite = SpriteCache.get(sprite2);
    }
    ...
    public static void addPrayer(int id, String tooltip, int width, int height, int glowSprite, int glowX, int glowY, int disabledSprite, int enabledSprite, int config, int configFrame, int hover) {
        ...
        widget.defaultText = tooltip;
        widget.hoverType = 52;
        widget.disabledSprite = SpriteCache.get(glowSprite);
        ...
        widget.tooltip = tooltip;
        widget.defaultText = tooltip;
        widget.disabledSprite = SpriteCache.get(disabledSprite);
        widget.enabledSprite = SpriteCache.get(enabledSprite);
    ...
    public static void addPrayer(int id, int configState, int configFrame, int requiredValues, int disabledSprite, int enabledSprite, String prayerName, int hover, int glowX, int glowY) {
        ...
        widget.opacity = 0;
        widget.hoverType = hover;
        widget.enabledSprite = SpriteCache.get(150);
        widget.width = 34;
        widget.height = 34;
        widget.spriteXOffset = glowX;
        widget.spriteYOffset = glowY;
    ...
    public static void configHoverButton(int id, String tooltip, int enabledSprite, int disabledSprite, int enabledAltSprite, int disabledAltSprite, boolean active, int... buttonsToDisable) {
        ...
        widget.width = widget.disabledSprite.width;
        widget.height = widget.enabledSprite.height;
        widget.enabledAltSprite = SpriteCache.get(enabledAltSprite);
        widget.disabledAltSprite = SpriteCache.get(disabledAltSprite);
    ...
    public static void addConfigSprite(int id, int spriteId, int spriteId2, int state, int config, String tooltip) {
        ...
        widget.valueIndexArray[0][2] = 0;
        widget.disabledSprite = spriteId < 0 ? null : SpriteCache.get(spriteId);
        widget.enabledSprite = spriteId2 < 0 ? null : SpriteCache.get(spriteId2);
    }
    ...
    public static void addInteractiveButton(int id, int enabledSpriteId, int disabledSpriteId, int width, int height, String text) {// hoverable
        // button
        ...
        tab.contentType = -1;
        tab.opacity = 0;
        tab.enabledSprite = imageLoaderNew(enabledSpriteId);
        tab.disabledSprite = imageLoaderNew(disabledSpriteId);
        tab.width = width;
        tab.height = height;
        tab.tooltip = text;
    ...
    public static void hoverButton10(int id, String tooltip, int enabledSprite, int disabledSprite, String buttonText,
        ...
        tab.opacity = (byte) 0;
        tab.hoverType = 52;
        tab.enabledSprite = SpriteCache.get(disabledSprite);
        tab.disabledSprite = SpriteCache.get(enabledSprite);
        tab.width = SpriteCache.getWidth(enabledSprite);
        tab.height = SpriteCache.getHeight(enabledSprite);
        tab.msgX = (tab.width / 2);
    ...
    public static void addHoverButton(int id, int spriteId, int clickX, int clickY, String text, int contentType, int hoverType, int action, int spriteX, int spriteY) {// hoverable
        // button
        Widget tab = addTabInterface(id);
        tab.id = id;
        tab.parent = id;
        ...
        tab.width = clickX;
        tab.height = clickY;
        tab.enabledSprite.setBounds(spriteX, spriteY);
        tab.disabledSprite.setBounds(spriteX, spriteY);
        tab.tooltip = text;
    }
    ...
    public static void addSprite(int id, int spriteId) {
        ...
        rsint.hoverType = 0;
        if (spriteId != -1) {
            rsint.enabledSprite = SpriteCache.get(spriteId);
            rsint.disabledSprite = SpriteCache.get(spriteId);
        }
    ...
    public static void addTitleText(int id, String text) {
        addText(id, text, fonts, 2, ColourConstants.DEFAULT_TEXT_COLOR, true);
    }
     ...
     */
    public static String getWrappedText(AdvancedFont tda, String text, int width) {
     ...
     * Return
     */
    public static Widget addWrappingText(int id, String text, AdvancedFont[] tda, int idx, int color, boolean center, boolean shadow, int width) {
        Widget tab = addTabInterface(id);
        tab.parent = id;
        tab.id = id;
        tab.type = TYPE_WRAPPING_TEXT;
        tab.optionType = 0;
        tab.width = width;
        tab.height = 11;
        tab.contentType = 0;
        ...
        tab.text_type = tda[idx];
        tab.defaultText = getWrappedText(tab.text_type, text, tab.width);
        return tab;
    }

    public static Widget addText(int id, int fontIdx, int color, boolean centered, String text) {
        return addText(id, text, fonts, fontIdx, color, centered);
    }
    ...
    public static void addBorder(int id, int width, int height) {
        ...
        tab.opacity = (byte) 0;
        tab.hoverType = 52;
        tab.enabledSprite = tab.disabledSprite = buildBackground(width, height, false);
        tab.width = width;
        tab.height = height;
    }
    ...
    public void addHorizontalDivider(int id, int width) {
        Widget tab = cache[id] = new Widget();
        tab.id = id;
        tab.parent = id;
        tab.type = 5;
        ...
        tab.opacity = (byte) 0;
        tab.hoverType = 52;
        tab.enabledSprite = tab.disabledSprite = buildHorizontal(width);
        tab.width = width;
        tab.height = 7;
    }

    public void addVerticalDivider(int id, int height) {
        Widget tab = cache[id] = new Widget();
        tab.id = id;
        tab.parent = id;
        tab.type = 5;
        ...
        tab.opacity = (byte) 0;
        tab.hoverType = 52;
        tab.enabledSprite = tab.disabledSprite = buildVertical(height);
        tab.width = 10;
        tab.height = height;
    }

    public SimpleImage buildVertical(int height) {
        int[][] pixels = new int[height][7];
        fillPixels(pixels, Client.autoBackgroundSprites[6], 0, 0, 7, height);
        return new SimpleImage(7, height, 0, 0, d2Tod1(pixels));
    }

    public SimpleImage buildHorizontal(int width) {
        int[][] pixels = new int[7][width];
        fillPixels(pixels, Client.autoBackgroundSprites[5], 0, 0, width, 7);
        return new SimpleImage(width, 7, 0, 0, d2Tod1(pixels));
    }
    ...
    public static SimpleImage buildBackground(int width, int height, boolean divider) {
        ...
        return new SimpleImage(width, height, 0, 0, d2Tod1(pixels));
    }
    ...
    public static void addButton(int id, int sprite) {
        ...
        tab.opacity = (byte) 0;
        tab.hoverType = 52;
        tab.enabledSprite = SpriteCache.get(sprite);
        tab.disabledSprite = SpriteCache.get(sprite);
        tab.width = SpriteCache.getWidth(sprite);
        tab.height = SpriteCache.getHeight(sprite);
        tab.tooltip = "";
    }
    ...
    public static void addToolTip(int id, String tooltip) {
        Widget tab = cache[id] = new Widget();
        tab.id = id;
        tab.parent = id;
        tab.type = 5;
    ...
    public static void addButtonWithMenu(int id, int sprite, String[] menu) {
        ...
        tab.opacity = (byte) 0;
        tab.hoverType = 17;
        if (SpriteCache.get(sprite) != null) {
            tab.enabledSprite = SpriteCache.get(sprite);
            tab.disabledSprite = SpriteCache.get(sprite);
        }
        tab.width = SpriteCache.getWidth(sprite);
    ...
    public static void sprite1(int id, int sprite) {
        ...
        class9.enabledSprite = SpriteCache.get(sprite);
    }

    public static void addActionButton(int id, int sprite, int sprite2, int width, int height, String s) {
        Widget rsi = cache[id] = new Widget();
        rsi.enabledSprite = SpriteCache.get(sprite);
        if (sprite2 == sprite)
            rsi.disabledSprite = SpriteCache.get(sprite);
        else
            rsi.disabledSprite = SpriteCache.get(sprite2);
        rsi.tooltip = s;
        rsi.contentType = 0;
        rsi.optionType = 1;
        rsi.width = width;
    ...
    protected static void addTransparentSprite(int id, int spriteId, int transparency) {
        ...
        tab.width = 512;
        tab.height = 334;
        tab.drawsTransparent = transparency < 100;
    }
    ...
    public static void addConfigButton(int ID, int pID, int disabledSprite, int enabledSprite, int width, int height, String tT,
        ...
        Tab.disabledSprite = SpriteCache.get(disabledSprite);// imageLoader(disabledSprite, bName);
        Tab.enabledSprite = SpriteCache.get(enabledSprite);
        Tab.tooltip = tT;
    }
    ...
    public static void addConfigButton(int id, int parent, int disabledSprite, int enabledSprite, String tooltip, int configId, int configState, int actionType) {
        ...
        widget.contentType = 0;
        widget.disabledSprite = disabledSprite < 0 ? null : SpriteCache.get(disabledSprite);
        widget.enabledSprite = enabledSprite < 0 ? null : SpriteCache.get(enabledSprite);
        widget.width = SpriteCache.getWidth(disabledSprite);
        widget.height = SpriteCache.getHeight(disabledSprite);
    ...
    public static void addTransButtonHover(int id, int sid, String tooltip, int transparency, boolean selected) {
        ...
        tab.opacity = (byte) 0;
        tab.hoverType = 52;
        tab.disabledSprite = SpriteCache.get(sid);// imageLoader(sid, spriteName);
        tab.enabledSprite = SpriteCache.get(sid);// imageLoader(sid, spriteName);
        tab.width = SpriteCache.getWidth(sid);
        tab.height = SpriteCache.getHeight(sid);
    ...
    public static void configHoverButton(int id, String tooltip, String spriteName, int sprite2, int sprite1, int enabledAltSprite, int disabledAltSprite, boolean active, int... buttonsToDisable) {
        ...
        tab.optionType = 1;
        tab.type = 11;
        tab.enabledSprite = imageLoader(sprite2, spriteName);
        tab.disabledSprite = imageLoader(sprite1, spriteName);
        tab.enabledSprite = SpriteCache.get(sprite1);
        tab.disabledSprite = SpriteCache.get(sprite2);
    ...
    public static void addHoveredConfigButton(Widget original, int ID, int IMAGEID, int disabledID, int enabledID) {
        ...
        if (disabledID != -1)
            hover.enabledSprite = SpriteCache.get(disabledID);
        if (enabledID != -1)
            hover.disabledSprite = SpriteCache.get(enabledID);
    ...
    public static void addHoverConfigButton(int id, int hoverOver, int disabledID, int enabledID, int width, int height, String tooltip, int[] valueCompareType, int[] requiredValues, int[][] valueIndexArray) {
        ...
        if (disabledID != -1)
            rsint.enabledSprite = SpriteCache.get(disabledID);
        if (enabledID != -1)
            rsint.disabledSprite = SpriteCache.get(enabledID);
    ...
    public static void addButton(int i, int parent, int sprite1, int sprite2, int hoverOver, String tooltip) {
        ...
        p.type = TYPE_SPRITE;
        p.type = 1;
        p.width = SpriteCache.getWidth(sprite1);
        p.height = SpriteCache.getHeight(sprite1);

        p.tooltip = tooltip;
        p.defaultText = tooltip;
        p.hoverType = hoverOver;
        p.disabledSprite = SpriteCache.get(sprite1);
        p.enabledSprite = SpriteCache.get(sprite1);
    }
    ...
    public static void addSpriteLoader(int childId, int spriteId) {
        ...
        rsi.optionType = 0;
        rsi.contentType = 0;
        rsi.enabledSprite = SpriteCache.get(spriteId);
        rsi.disabledSprite = SpriteCache.get(spriteId);
        rsi.width = SpriteCache.getWidth(spriteId);
        rsi.height = SpriteCache.getHeight(spriteId) - 2;
    ...
    public static void adjustableConfig(int id, String tooltip, int sprite, int opacity, int enabledSpriteBehind, int disabledSpriteBehind) {
        ...
        tab.disabledSprite = SpriteCache.get(sprite);
        tab.enabledAltSprite = SpriteCache.get(enabledSpriteBehind);
        tab.disabledAltSprite = SpriteCache.get(disabledSpriteBehind);

        tab.width = SpriteCache.getWidth(enabledSpriteBehind);
        tab.height = SpriteCache.getHeight(enabledSpriteBehind);
    ...
    /*public static Widget createSprite(final int parentIndex, final int index, final int disabled, final int enabled) {
        ...
        final SimpleImage disabledSprite = SpriteCache.get(disabled);
        final SimpleImage enabledSprite = SpriteCache.get(enabled);
    ...
    public static Widget createSprite(final int parentIndex, final int index, final int disabled, final int enabled) {
        final SimpleImage spriteDisabled = SpriteCache.get(disabled);
        final SimpleImage spriteEnabled = SpriteCache.get(enabled);
        final Widget sprite = cache[index] = new Widget();
        sprite.id = index;
        sprite.parent = parentIndex;
        sprite.type = 5;
        sprite.width = SpriteCache.getWidth(disabled);
        sprite.height = SpriteCache.getHeight(disabled);
    ...
    public static void addHoverImage_sprite_loader(int id, int spriteId) {
        ...
        tab.optionType = 0;
        tab.contentType = 0;
        //int w = SpriteCache.get(spriteId).myWidth;
        //int h = SpriteCache.get(spriteId).myHeight;
        tab.width = 512;
        tab.height = 334;
        tab.opacity = 0;
        tab.hoverType = 52;
    ...
    static SimpleImage imageLoaderNew(int spriteId) {
        ...
        try {
            ...
            sprite = SpriteCache.get(spriteId);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public void child(int id, int widgetId, int x, int y) {
        ...
        if (Widget.PRINT_INFO) {
            var toSet = Widget.cache[widgetId];
            if (toSet != null)
                toSet.print();
        }
    }
        ...
        if (model == null) {
            ...
            switch (model_type) {
                case 1:
                    ...
                    break;
                case 2:
                    var12 = NpcDefinition.get(model_id).get_dialogue_model();
                    break;
                case 3:
                    var12 = Client.local_player != null ? Client.local_player.get_dialogue_model() : null;
                    break;
                case 4:
                    ItemDefinition var15 = ItemDefinition.get(model_id);
                    var12 = var15.get_widget_model(50);
                    var13 += var15.ambient;
    ...
    public static void addLunarHoverBox(int interface_id, int spriteOffset) {
        ...
        RSInterface.enabledSprite = SpriteCache.get(LUNAR_HOVER_BOX_SPRITES_START + spriteOffset);
    ...
    public static void addLunarRune(int i, int spriteOffset, String runeName) {
        ...
        RSInterface.opacity = 0;
        RSInterface.hoverType = 52;
        RSInterface.enabledSprite = SpriteCache.get(LUNAR_RUNE_SPRITES_START + spriteOffset);
        RSInterface.width = 500;
        RSInterface.height = 500;
    }
    ...
    public static void addLunar2RunesSmallBox(int ID, int r1, int r2, int ra1, int ra2, int rune1, int lvl, String name,
        ...
        rsInterface.disabledSprite = SpriteCache.get(LUNAR_ON_SPRITES_START + spriteOffset);
        rsInterface.enabledSprite = SpriteCache.get(LUNAR_OFF_SPRITES_START + spriteOffset);
    ...
    public static void addHoverButtonWConfig(int i, int spriteId, int spriteId2, int width, int height, String text,
        ...
        if (spriteId2 != -1) {
            tab.enabledSprite = SpriteCache.get(spriteId2);
        }
    }
    ...
    public static void configHoverButtonSpriteOutline(int id, String tooltip, int enabledSprite, int disabledSprite,
        ...
        tab.disabledSprite = SpriteCache.get(enabledSprite);
        tab.enabledSprite = SpriteCache.get(enabledSprite);
        tab.spriteWithOutline = SpriteCache.get(sprite);
        tab.width = tab.disabledSprite.width;
        tab.height = tab.enabledSprite.height;
        tab.enabledAltSprite = SpriteCache.get(disabledSprite);
        tab.disabledAltSprite = SpriteCache.get(disabledSprite);
        tab.buttonsToDisable = buttonsToDisable;
        tab.active = active;
        tab.spriteOpacity = 255;
    }
    ...
    public static void drawPixels(int interfaceId, int width, int height, int colour, int borderColor, int transparency) {
        Widget component = addInterface(interfaceId);
        component.id = interfaceId;
        component.type = TYPE_DRAW_PIXELS;
        component.width = width;
        component.height = height;
        component.fillColor = colour;
        component.borderColor = borderColor;
        component.opacity = (byte) transparency;
    }

    public static Widget addRectangle(int id, int width, int height, int colour, int alpha, boolean filled) {
        Widget tab = cache[id] = new Widget();
        tab.textColour = colour;
        tab.filled = filled;
        tab.id = id;
    ...
    public static void addSprites(int id, int... spriteIds) {
        ...
        for (int i = 0; i < spriteIds.length; i++) {
            SpriteCache.get(spriteIds[i]);
            component.backgroundSprites[i] = SpriteCache.get(spriteIds[i]);
        }
    ...
    public static void slider(int id, double min, double max, int icon, int background) {
        Widget widget = addInterface(id);
        widget.slider = new Slider(SpriteCache.get(icon), SpriteCache.get(background), min, max);
    ...
    protected static Widget addContainerNoActions(int id, int contentType, int maxItemsOnSingleRow, int rows, int xPad, int yPad, int opacity, boolean move, boolean displayAmount) {
     ...
     */
    public static Widget addContainer(int id, int contentType, int maxItemsOnSingleRow, int rows, int xPad, int yPad, String... actions) {
    ...
    protected static Widget addContainerNoActions(int id, int width, int height, int xPad, int yPad, boolean move, boolean displayAmount) {
        return addContainerNoActions(id, 0, width, height, xPad, yPad, 0, move, displayAmount);
    }
    ...
    public static void addItemGroup(int id, int itemsPerRow, int itemsInColumn, int xPadding, int yPadding, String... actions) {
        Widget widget = addInterface(id);
        widget.width = itemsPerRow;
        widget.height = itemsInColumn;
        widget.inventoryItemId = new int[5000];
        widget.inventoryAmounts = new int[5000];
        widget.usableItems = false;
        widget.hasActions = false;
        widget.inventoryMarginX = xPadding;
        widget.inventoryMarginY = yPadding;
        widget.inventoryOffsetX = new int[20];
        widget.inventoryOffsetY = new int[20];
        widget.sprites = new SimpleImage[20];
        widget.type = 2;
    ...
    public static void itemGroup(int id, int itemsPerRow, int spaceBetweenRows, int xPadding, int yPadding) {
        Widget widget = addInterface(id);
        widget.width = itemsPerRow;
        widget.height = spaceBetweenRows;
        widget.inventoryItemId = new int[5000];
        widget.inventoryAmounts = new int[5000];
        widget.usableItems = false;
        widget.hasActions = false;
    ...
    public static void addColorBoxNonTransparent(int id, int color, int width, int height) {
        Widget widget = addInterface(id);
        widget.width = width;
        widget.height = height;
        widget.color = color;
    ...
    public static void addSpellSmall2_3(int interfaceId, int rune, int secondaryRune, int thirdRune, int fourthRune, int runeAmount1, int runeAmount2, int runeAmount3, int runeAmount4, int runeChild, int secondaryRuneChild, int thirdRuneChild, int fourthRuneChild, int levelRequired, String spellName, String spellDescription, AdvancedFont[] TDA, int spellOffSprite, int spellOnSprite, int selectedMask, int type) {
        ...
        widget.enabledSprite = SpriteCache.get(spellOffSprite);
        widget.disabledSprite = SpriteCache.get(spellOnSprite);
    ...
    protected static void addFamiliarHead(final int interfaceID, final int width, final int height, final int zoom) {
        ...
        widget.type = 6;
        widget.defaultMediaType = 2;
        widget.defaultMedia = 2449;
        widget.modelZoom = zoom;
        widget.modelAngleX = 40;
        widget.modelAngleY = 1800;
        widget.height = height;
        widget.width = width;
    }
    ...
    public static void addButton1(int i, int parent, int w, int h, int sprite1, int sprite2, int hoverOver, String tooltip, int itemId, boolean geSearch) {
        ...
        ;
        p.disabledSprite = SpriteCache.get(sprite1);
        p.enabledSprite = SpriteCache.get(sprite2);
        p.itemId = itemId;
    ...
    public static void addPet(int ID) {
        ...
        petCanvas.modelAngleX = 150;
        petCanvas.modelAngleY = 0;
    }
    ...
    public static void addItemModel(int interfaceId, int itemId, int w, int h, int zoom) {
        ...
        widget.modelAngleX = itemDef.xan2d;
        widget.modelAngleY = itemDef.yan2d;
        widget.type = 6;
        widget.defaultMediaType = 4;
        widget.defaultMedia = itemId;
        widget.modelZoom = zoom;
        widget.height = h;
        widget.width = w;
    }
    ...
    public String toString() {
        return "Widget{" +
                ...
                ", textSpriteClicked=" + textSpriteClicked +
                ", spriteWithOutline=" + spriteWithOutline +
                ", enabledAltSprite=" + enabledAltSprite +
                ", disabledAltSprite=" + disabledAltSprite +
                ", disabledSprite=" + disabledSprite +
                ", enabledSprite=" + enabledSprite +
                ...
                ", text_type=" + text_type +
                ", opacity=" + opacity +
                ", buttonsToDisable=" + Arrays.toString(buttonsToDisable) +
                ", dropdownColours=" + Arrays.toString(dropdownColours) +
                ", itemOpacity=" + Arrays.toString(itemOpacity) +
                ", id=" + id +
                ", inventoryAmounts=" + Arrays.toString(inventoryAmounts) +
                ", inventoryItemId=" + Arrays.toString(inventoryItemId) +
                ...
                ", clickSprite1=" + clickSprite1 +
                ", modelType=" + modelType +
                ", modelFileId=" + modelFileId +
                ", itemId=" + itemId +
                ", childToIntersect=" + childToIntersect +
                ", positionX=" + positionX +
                ", positionY=" + positionY +
                ", radioID=" + radioID +
                ", clickSprite2=" + clickSprite2 +
                ", hoveredOutlineSpriteXOffset=" + hoveredOutlineSpriteXOffset +
                ", hoveredOutlineSpriteYOffset=" + hoveredOutlineSpriteYOffset +
                ", modelZoom=" + modelZoom +
                ", modelAngleX=" + modelAngleX +
                ", modelAngleY=" + modelAngleY +
                ", modelRotation3=" + modelRotation3 +
                ", child_y=" + Arrays.toString(child_y) +
                ...
                ", secondaryAnimationId=" + secondaryAnimationId +
                ", secondaryHoverColor=" + secondaryHoverColor +
                ", children=" + Arrays.toString(children) +
                ", child_x=" + Arrays.toString(child_x) +
                ", inventoryMarginY=" + inventoryMarginY +
                ", valueCompareType=" + Arrays.toString(valueCompareType) +
                ", currentFrame=" + currentFrame +
                ", inventoryOffsetY=" + Arrays.toString(inventoryOffsetY) +
                ", hoverType=" + hoverType +
                ...
                ", parent=" + parent +
                ", selectedTargetMask=" + selectedTargetMask +
                ", requiredValues=" + Arrays.toString(requiredValues) +
                ", contentType=" + contentType +
                ", inventoryOffsetX=" + Arrays.toString(inventoryOffsetX) +
                ", alpha=" + alpha +
                ...
                ", valueIndexArray=" + Arrays.toString(valueIndexArray) +
                ", pauseTicks=" + pauseTicks +
                ", color=" + color +
                ", textClickedX=" + textClickedX +
                ", textClickedY=" + textClickedY +
                ", characterLimit=" + characterLimit +
                ", buttons=" + Arrays.toString(buttons) +
                ", borderWidth=" + borderWidth +
                ", borderColor=" + borderColor +
                ", progressBackColor=" + progressBackColor +
                ", progressBackAlpha=" + progressBackAlpha +
                ", customOpacity=" + customOpacity +
                ", drawProgressText=" + drawProgressText +
                ", hoverHasText=" + hoverHasText +
                ", hoverScrollBar=" + hoverScrollBar +
                ", isInFocus=" + isInFocus +
                ", displayAsterisks=" + displayAsterisks +
                ", updatesEveryInput=" + updatesEveryInput +
                ", updateConfig=" + updateConfig +
                ", textIsClicked=" + textIsClicked +
                ...
                ", displayExamine=" + displayExamine +
                ", allowSwapItems=" + allowSwapItems +
                ", invisible=" + invisible +
                ...
                ", hovered=" + hovered +
                ", inverted=" + inverted +
                ", isAchievementHover=" + isAchievementHover +
                ", isHovered=" + isHovered +
                ", fullCint=" + fullCint +
                ", drawNumber=" + drawNumber +
                ", geSearchButton=" + geSearchButton +
                ", isRadioButton=" + isRadioButton +
                ", filled=" + filled +
                ", centerText=" + centerText +
                ", rightText=" + rightText +
                ", serverCheck=" + serverCheck +
                ", regularHoverBox=" + regularHoverBox +
                ", clickable=" + clickable +
                ", drawsTransparent=" + drawsTransparent +
                ", fancy=" + fancy +
                ", hightDetail=" + hightDetail +
                ", hoverText='" + hoverText + '\'' +
                ", defaultInputFieldText='" + defaultInputFieldText + '\'' +
                ", spellName='" + spellName + '\'' +
                ", tooltip='" + tooltip + '\'' +
                ...
                ", backingColour=" + backingColour +
                ", barColour=" + barColour +
                ", childCount=" + childCount +
                ", backgroundSprites=" + Arrays.toString(backgroundSprites) +
                ", tooltips=" + Arrays.toString(tooltips) +
                ", newScroller=" + newScroller +
                ", drawInfinity=" + drawInfinity +
                ", spritesToDraw=" + Arrays.toString(spritesToDraw) +
                ", textToDraw=" + Arrays.toString(textToDraw) +
    ...
    public void print() {
        //log.info("{}", this);
        log.info("{} {}", id, defaultText);
    }

    protected void enablePrints() {
        Widget.PRINT_INFO = true;
    }
    protected void printsOff() {
        Widget.PRINT_INFO = false;
    }
}
